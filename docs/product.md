...
| 版號 | 日期 | 主要內容 |
|------|------|---------|
| v1.21.0 | 2026-04-26 | 常規動作預設設定：建立/編輯常規時可設定每個動作的預設單位（下/趟/秒/距離）、組數與次數；載入常規時自動預填；伸展動作智慧預設（繩子伸展拆分為前後/8字型、棍子伸展改時長制等） |
| v1.20.0 | 2026-04-26 | 自訂訓練常規：建立、儲存、一鍵載入有序動作清單；動作可左滑刪除、拖曳排序 |
| v1.19.0 | 2026-04-26 | 週訓練詳情全螢幕頁：點擊儀表板本週 vs 上週卡片，可查看本週/上週完整訓練紀錄、部位覆蓋狀況及久未訓練動作 |
| v1.18.7 | 2026-04-26 | 修正訓練日誌卡片：動作名稱過長時按鈕不再縱向排列 |
| v1.18.6 | 2026-04-26 | 修正編輯訓練動作選擇器：有分類的自訂動作現在出現在對應分類下而非「我的自訂動作」 |
| v1.18.5 | 2026-04-26 | 修正動作分類篩選器顯示錯誤分類動作的問題 |
| v1.18.4 | 2026-04-26 | 新增用戶自訂動作至系統預設分類（新增「伸展」分類，補齊多個現有分類動作），修正 constants.js 與 FitForge.jsx 不同步問題 |
| v1.18.3 | 2026-04-25 | 教練課日 day row emoji 動態化：全部自練時顯示 💪 而非 🏅，避免誤解 |
| v1.18.2 | 2026-04-25 | 訓練日誌 UX 重構：移除重複當日紀錄卡片、日曆點擊連動歷史列表篩選、強化週/日折疊層級視覺 |
| v1.18.1 | 2026-04-24 | 教練課項目標記改為雙狀態徽章（🏅 上課 / 💪 自練），切換時顯示 toast 提示 |
| v1.18.0 | 2026-04-24 | 教練課當天每筆訓練可個別標記是否為上課內容，點擊 🏅 切換；取消整天標記時自動清除所有項目的教練課狀態 |
| v1.17.0 | 2026-04-24 | 教練課動作庫新增三種瀏覽模式：全部動作、依課程時間（聚焦筆記）、依部位分類篩選 |
| v1.16.0 | 2026-04-20 | 儀表板新增本週 vs 上週訓練量對比卡片；目標接近達標（≥90%）顯示「快到了 🎯」橘色徽章 |
| v1.15.2 | 2026-04-19 | 修復儀表板「近期訓練」跑步機顯示空白條的問題，正確顯示時間、距離、配速等有氧資料 |
| v1.15.1 | 2026-04-19 | 修復自訂動作在 PR 部位篩選中消失的問題 |
| v1.15.0 | 2026-04-19 | 今日訓練建議加入部位篩選；個人最佳 PR 全螢幕頁加入部位分類篩選 |
| v1.14.1 | 2026-04-18 | 跑步機記錄重新設計：分秒輸入取代速度欄位，自動計算並顯示配速（05:43 /km 格式） |
| v1.14.0 | 2026-04-18 | 訓練動作自動帶入上次組數與重量；身材記錄超過 5 筆自動折疊，「顯示更多」一鍵展開完整歷史 |
| v1.13.0 | 2026-04-10 | FAB 速撥選單 + 今日訓練建議：久未訓練動作一鍵加入今日計畫，均衡訓練各部位 |
| v1.12.0 | 2026-04-08 | 教練課學習記錄：動作庫瀏覽所有教練課動作與筆記、課堂記錄依日期展開每堂課內容 |
| v1.11.2 | 2026-04-08 | 修正 iOS PWA 頂部與狀態列重疊問題，加入 safe-area-inset-top 適配 |
| v1.11.1 | 2026-04-08 | 修正個人最佳 PR 全螢幕頁可讀性：動作名稱顏色、日期對比度、卡片邊框分隔、展開指示箭頭 |
| v1.11.0 | 2026-04-08 | Dashboard PR 卡片重設計：顯示最新 3 筆破紀錄動作，「查看全部」跳出全螢幕 PR 頁，點擊動作可展開重量趨勢折線圖 |
| v1.10.1 | 2026-04-07 | 修正教練課日期列在手機寬度下折行的問題，改為 emoji 指示 + 圓形圖示按鈕 |
| v1.10.0 | 2026-04-07 | 教練課標記功能：訓練日誌可標記教練課、Dashboard 進度條追蹤扣打用量 |
| v1.9.4 | 2026-04-04 | 帳號設定新增連續天數重置按鈕，可手動修正錯誤的連續紀錄 |
| v1.9.3 | 2026-04-04 | 修正行事曆同步靜默失敗問題：同步失敗時顯示錯誤提示，並防止 PWA 快取舊的課程資料 |
| v1.9.2 | 2026-04-03 | 修正計時器編輯時長後需重開才生效的問題，確認後立即套用新時長 |
