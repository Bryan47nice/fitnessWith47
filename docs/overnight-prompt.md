...
2. **backlog 有 `✅ [用戶核准]`** → 優先度次高，讀取對應 `docs/plans/` 規劃文件後嚴格按規劃實作
3. **backlog 有 `📋 [需規劃]`** → 不實作，改為產出規劃文件（見下方「複雜功能規劃流程」）
4. **backlog 有 `⏳ Pending`** → 依順序取題（用戶排序代表優先度）
5. **backlog 是空的（第一次執行）** → 自判選題（見下方規則），並將完整清單寫入 backlog
...
1. 產出規劃文件，寫入 `docs/plans/YYYY-MM-DD-{feature-name}.md`，內容包含：
   - **功能目標**：一句話說明要解決什麼問題
   - **影響範圍**：哪些檔案、元件、資料結構需要異動
   - **Firestore schema 異動**（如有）
   - **分階段實作建議**：若複雜度高，建議拆成哪幾個 branch 依序做
   - **風險與注意事項**：可能的邊際案例或破壞性改動
2. 更新 backlog：將 `📋 [需規劃]` 改為：
   ```
   - 📝 [規劃完成] {功能描述} → docs/plans/YYYY-MM-DD-{feature-name}.md
   ```
3. 在 overnight report 中新增「📋 規劃待核准」區塊（格式見 report 範本）
...
→ 核准請說「approve {feature-name}」；退回或修改請說「reject {feature-name} {修改意見}」
