...
- ⏳ [高] 伸展流程範本：用戶可預設一套專屬伸展動作順序，訓練時一鍵套入當日記錄，套入後可臨時增刪調整
- ⏳ [高] 今日訓練建議：加入部位篩選，讓用戶可先選今天想練的部位（胸/背/腿等），建議清單只顯示對應動作
- ⏳ [高] 個人最佳 PR：加入部位分類篩選（胸/背/肩/腿/手臂/有氧等），方便查看特定部位 PR；補上有氧 PR（跑步機/慢跑等最佳距離或配速）
- ⏳ [中] 儀表板：本週 vs 上週訓練量對比卡片 — 讓用戶一眼看到訓練量趨勢是否在提升
